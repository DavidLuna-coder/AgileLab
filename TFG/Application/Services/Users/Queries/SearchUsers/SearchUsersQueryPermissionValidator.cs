...
{
    public class SearchUsersQueryPermissionValidator(IUserInfoAccessor userInfoAccessor, ApplicationDbContext context) : IPermissionValidator<SearchUsersQuery>
    {
        private readonly Permissions requiredPermissions = Permissions.ViewUsers;

        public async Task<bool> HasPermissionAsync(SearchUsersQuery request)
        {
