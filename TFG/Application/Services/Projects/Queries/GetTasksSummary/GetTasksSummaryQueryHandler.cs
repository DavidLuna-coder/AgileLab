...
{
	public class GetTasksSummaryQueryHandler(ApplicationDbContext dbContext, IOpenProjectClient openProjectClient)
	{
		public async Task<PaginatedResponseDto<TaskSummaryDto>> Handle(GetTasksSummaryQuery request)
		{
			Project project = dbContext.Projects
							  .FirstOrDefault(p => p.Id == request.ProjectId) ?? throw new NotFoundException("Project not found");
			...
			OpenProjectClient.Models.OpenProjectCollection<GitlabMergeRequest>[] mergeRequests = await Task.WhenAll(tasks);
